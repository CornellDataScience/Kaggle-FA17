...
class RPNRegressionLoss(keras.layers.Layer):
    ...
    def compute_loss(output, target, labels):
        ...
        p_star_i = tensorflow_backend.where(keras.backend.greater(labels, 0), keras.backend.ones_like(labels), keras.backend.zeros_like(labels))

        p_star_i = keras.backend.expand_dims(p_star_i, 0)

        a_y = tensorflow_backend.smooth_l1(keras.backend.expand_dims(output, 0), keras.backend.expand_dims(target, 0), anchored=True)
