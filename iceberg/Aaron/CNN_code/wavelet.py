...
train_df, train_images_raw = load_and_format(train_path)
test_df, test_images_raw = load_and_format(test_path)
...
#Wavelet
train_images = wavelet.wavelet_mean_df(train_images_raw[:, :, :, 0], train_images_raw[:, :, :, 1])

#Wavelet
test_images = wavelet.wavelet_mean_df(test_images_raw[:, :, :, 0], test_images_raw[:, :, :, 1])
...
image_input = Input(shape=(38, 38, 1), name="image")
